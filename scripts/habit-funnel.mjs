...
//   body_read→ knowledge_body_read         (native Read of store body = 翻库; KT-DEC-0030)
//              (+ legacy knowledge_consumed for old telemetry)
//   select   → knowledge_selection         (AI picked specific entries; may be sparse post-lean)
//   fetch    → knowledge_sections_fetched  (legacy two-step body fetch)
//   edit     → file_mutated / edit_intent_checked (work happened against KB)
//   archive  → cite_policy_activated        (cite/archive policy engaged)
//
// Floor: surface≥1 and body_read≥1 — KB pushed AND actually opened.
// Pre-lean ledgers used knowledge_consumed; counting only that falsely fails
// modern dogfood (consume==0 while knowledge_body_read>0).
//
...
const bodyRead =
  (counts.knowledge_body_read || 0) + (counts.knowledge_consumed || 0);

const funnel = {
  surface: counts.hook_surface_emitted || 0,
  plan: counts.knowledge_context_planned || 0,
  body_read: bodyRead,
  body_read_native: counts.knowledge_body_read || 0,
  body_read_legacy_consumed: counts.knowledge_consumed || 0,
...
// 翻库率 proxy: bodies opened / surfaced.
const recallHitRate =
  funnel.surface > 0 ? +(funnel.body_read / funnel.surface).toFixed(2) : null;

// Floor: the load-bearing stages must be alive.
const FLOOR = { surface: 1, body_read: 1 };
...
const baseline = {
  ...
  funnel,
  recall_hit_rate_body_read_over_surface: recallHitRate,
  floor: FLOOR,
  floor_status: belowFloor.length === 0 ? "pass" : "below-floor",
  nonzero_stages: Object.values(funnel).filter((v) => typeof v === "number" && v > 0).length,
  ...
  note: "body_read = knowledge_body_read + legacy knowledge_consumed (lean recall era)",
};
...
console.log(`\n  翻库率 (body_read/surface):     ${recallHitRate}`);
console.log(`  non-zero stages:               ${baseline.nonzero_stages}/${baseline.total_stages}`);
console.log(`  floor (surface≥1, body_read≥1): ${baseline.floor_status}`);
console.log(`  baseline → ${OUT}`);

if (belowFloor.length > 0) {
  console.error(
    `\nG-HABIT FAIL: habit loop below floor — ${belowFloor.map(([k]) => k).join(", ")} == 0 (knowledge loop dead or ledger misaligned)`,
  );
  process.exit(1);
}
console.log(
  `\nG-HABIT PASS: activation funnel alive — KB pushed (${funnel.surface}) AND opened (${funnel.body_read}), ${baseline.nonzero_stages} non-zero stages (cc dogfood; codex deferred)`,
);
