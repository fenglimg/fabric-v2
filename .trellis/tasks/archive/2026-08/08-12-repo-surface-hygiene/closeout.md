...
| 项 | 状态 |
|---|---|
> ⚠️ 本表在任务收口后被回填过一轮。下面每行都标了**当前**状态,不是收口那一刻的状态。

| 项 | 状态 |
|---|---|
| ~~`C:` 垃圾的**源头**~~ | **划掉 — 前提被推翻**。上面第 22-27 行三条证据证明不存在"未修的源头":`git log --all` 无记录、唯一带该字面量的测试写路径全被 mock、多轮全量跑零重现。 |
| `assets/brand/*.svg` 仓外用途 | 未向用户确认,默认保留 |
| ~~那 197 个不被类型检查的测试文件~~ | **已闭环**:`pnpm typecheck:tests` 棘轮门禁已进 CI(`scripts/typecheck-tests.mjs` + 基线 29 文件 / 120 错)。剩下的是那 120 条基线错,只许降不许升。 |
| ~~`packages/server/.ccw/.spec-index/*.json`~~ | **已闭环**:跟踪计数已归 0,`.ccw/` 也进了 `.gitignore`。 |
| ~~knip 的 2 条既存未用依赖~~ | **已闭环**:合并后暴露成 CI 红(W1 删掉唯一静态 import 的 probe 后 knip 才看见),已在 `knip.config.ts` 白名单说明清楚,`pnpm lint` PASS。 |
