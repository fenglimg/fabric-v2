...
> ⛔ **已终局裁决(2026-08-12,用户拍板): 不开禁,且理由从「暂缓」升级为永久拒绝。已归档为 `KT-DEC-0090`(取代 KT-DEC-0089 裁决 #1),防复议。以下材料保留作论证留痕。**
>
> **复评拿到的数据**(W3 于 08-11 收口,其后 1.5 天 / 7 session / 1032 条账本事件): `knowledge_context_planned` **214 次,214 次全部带 `target_paths`** —— 下方「知识浮现只有路径+session 两个锚」这条立论里的 path 锚**从未缺席、从未降级**。同期 `knowledge_proposed` 43 / `knowledge_promoted` 29,证明 W3 的收口仪式与快速通道确在产出。
>
> **终局理由与 §3.2 同构**: §3.2 被否是因为「本仓已装 `trellis mem`,再做是重复建设」。任务轴一模一样 —— **产出这份 roadmap 的任务本身就跑在 `.trellis/tasks/08-10-trellis-gap-optimization/` 里,跨三天七个会话,任务锚一直在且好用,只不过归 Trellis 管**。Fabric 再长一根即 KT-PIT-0058 false-friend 教科书案例。
>
> **数据同时纠正了缺口定位**: 真短板不是「锚不够多」,是 `session_archive_attempted` 3 次 vs `session_ended` 13 次(归档记账覆盖率 **23%**)。加任务轴治不了它 —— 那是触达率问题,**已另开议题**。
>
> **将来若仍有需求,唯一可接受形态是「读」不是「建」**: 让 `fab_recall` 把当前 Trellis task 目录当作额外 path 锚(零新概念 / 零状态机 / 不碰 AGPL)。新建任务生命周期、工件体系、journal 一律仍在 KT-DEC-0078 hard don't-steal 封印内。
...
- ~~**B10 server services/ 平铺治理**~~ ❌ **审计后不做**(2026-08-12 裁决)。原提案是按前缀落子目录,纯移动+改 import。**收益侧站不住**: 提案自己承认「纯移动」—— 零行为变化、零缺陷密度变化,唯一产出是目录树好看一点。而 `doctor-*` 这个前缀本身已经在扮演目录的角色: 46 个文件在任何按名排序的视图里天然聚成一簇,`Glob doctor-*` 与 `Glob doctor/**` 检索力等价。**代价侧是实的**: 96 个文件的路径全变 → 整个包的 `git blame` / `git log --follow` 断链,而这个包刚经历 B11 两轮相位分解,blame 正是接下来判断「这段逻辑为什么长这样」的主要工具。判据同 B11: **文件位置该不该动,看的是「它里面有没有不属于它的东西」,不是「同级文件有几个」** —— B10 没提出任何一个错位文件。
  - 🔄 **同日下午翻案并已实施**(`55610a89` + `4e567575`,任务 `08-12-repo-surface-hygiene`)。**翻案依据不是新证据,是价值函数变了**: 上面这段裁决全程只在「正确性 / 缺陷密度」一个维度下称量,而用户随后明确把「工程美感 / 一眼看到结构」提为一等诉求 —— 按 KT-GLD-0011,旧决策的 rationale 不再适配当前需求时不得拿它挡。**不静默改口,两处双向记录。**
  - 实施时原提案也被推翻了一半: **「按前缀落子目录」经 import 图检验不成立**。前缀分布是 `doctor 48 | review 8 | plan 8 | 然后 30 个单例`,照做等于 3 个目录 + 30 个散件。改用标签传播社区发现(不读文件名)+ 内聚比判据,只有 `doctor/` 2.00 与 `review/` 1.18 成立;`plan-context/` 0.71、`retrieval/` 0.03、`ledger/` 0.13、`knowledge/` **0.00**、`store/` 0.08 全部否掉 —— `knowledge` 那七个「听起来天然是一伙」的文件彼此零引用,建这个目录名就是撒谎。**前缀是命名约定,不是依赖事实。**
  - 上面「实测 96 文件」也低估了:真实是 **179 个条目**(源码 97 / 共址测试 80 / 目录 2)。迁移后 **179 → 88**。断 blame 的代价用 `git mv` 对冲(92 个 rename 全部命中,`git log --follow` 可追)。行为等价判据是**三包用例数与基线逐位相等**(646/6、1159/2、1473/5),不是「全绿」。
...
1. ~~**§3.1 任务轴重议**~~ ✅ **已裁决(2026-08-12): 不开禁,永久封印**,归档为 `KT-DEC-0090`(取代 KT-DEC-0089 裁决 #1,并已回写 0089 的 summary + `related` 防止它继续被读成待议)。论证与数据见 §3.1 顶部的 ⛔ 块。
2. **归档积压** —— §0 排的是「W3 之后跑」,一直未动,计数已从 44 → 47。**写入侧已清空(2026-08-12): 9 条 pending 全部 approve 入库(KT-GLD-0021/0022、KT-PIT-0076~0081、KT-DEC-0090),pending 归零**;跨会话补扫仍待定 —— 增量扫描口径下只有 4 个会话在范围内,47 是「历史上有高价值改动但没归档过」的另一个口径,要吃掉它得显式 `range: "all"`,预期大比例会被语义去重 reject。
   **❌ 裁决:不做(2026-08-12)。** 成本是 47 个会话的全量扫描 + 逐条语义去重,收益是
   一批「当时没归档、现在也未必还成立」的历史条目 —— 而知识库的价值在于**下次会被读到**,
   不在于条目数。真有价值的老教训会在下次踩到时自然重新浮现并归档,那时它还自带新鲜的
   复现证据。**把 47 这个数字当欠债看是误判:它是一个计数口径,不是一堆待办。**
3. ~~**(新议题,已从本任务分出)归档记账覆盖率 23%**~~ ❌ **前提不成立,撤销(2026-08-12 复核)**。
   `session_ended` **不是每会话一次** —— 它每次 Stop 都发,全账本 341 个事件只涉及 203 个会话,
   均值 1.68,分布 `{1次:152, 2次:20, 3次:10, 4次:5, 5次以上:16}`。所以「13 : 3 = 23%」
   是拿 **per-stop 事件数**当分母去除 **per-archive-run 事件数**,两个口径不同源,比值没有意义。
   按会话去重后的真实数是 **91 / 203 = 45%**。
   更进一步,45% 也不是「缺口」——大量会话本就没有可归档内容,`fabric-archive` 的
   viability gate 是**故意**空转的;真要衡量得问「有归档信号的会话里有多少记了账」,
   而账本里没有「有信号」这个字段,答不了。
   **另一个独立发现**:有 24 个会话有 `session_archive_attempted` 却**没有** `session_ended`
   —— 两个信号都不是可靠的会话计数器,谁当分母都会得出假结论。
   *教训:算一个比值之前先确认分子分母是不是同一个计数单位。这条与 §3.1「214/214 recall
   都带 target_paths 所以缺锚前提坍塌」是同一类 —— **议题的前提本身要先被量一次**。*
   <br>~~原文~~: —— 复评任务轴时的账本数据顺带暴露: W3 后 `session_ended` 13 次而 `session_archive_attempted` 只有 3 次。可能是 Stop hook 没触发 / skill 早退没记账 / 归档真没发生,三种处置完全不同。**这是 W3「沉淀减负」的效果验收缺口,不属于 Trellis 对照这个任务的范围**,另开。
