...
- [ ] W2-1 呈现注册表：`{ key → tier }` + 预设常量表。**未登记键自动落「高级」**（不变量）。
- [ ] W2-2 常用 / 高级两层 + 搜索框（命中高级项时自动展开高级区）。
- [ ] W2-3 已修改竖条 + 悬停「恢复默认」（接 W0）。来源层降为详情。
- [ ] W2-4 预设卡：`POST /api/config/preset` 服务端逐键走 `writeFieldValue`；读取时逐键全等反推当前档位，不匹配显示「自定义」；**不落 `preset_name` 字段**。
...
- [ ] W2-7 文案语域统一：陈述句 + 说清后果 + 给出实际数值（参照 VS Code 设置 / Raycast 手册），去掉口语化修辞。
- [ ] 验证：`cd packages/cli && pnpm exec vitest run __tests__/console-global-config-*.test.ts`
- [ ] 测试要点（对齐 KT-GLD-0019 变异判据）：① 塞一个未登记键，断言它出现在高级区而非消失；② 改预设覆盖的任一键，断言档位显示变「自定义」；③ reset 后该项竖条消失。
