...
| skill `templates/skills/*/SKILL.md` | 6 | 全部安装且被引用;2 个 thin shim 待裁决(见 I3) |
...
| 永久清扫清单 `DEPRECATED_SKILL_DIRS` | 10 | 只增不减,无退役判据(见 I4) |
...
## 4. I3 — 两个 thin shim skill 的常驻描述税(待裁决)
...
**需用户裁决**,不自作主张 —— 这是产品面(AI 的技能选择体验),不是纯技术清理。

## 5. I4 — DEPRECATED_SKILL_DIRS 只增不减(待裁决)
...
**需用户裁决**:(a) 零用户阶段直接清空;(b) 定一条退役规则(如「跨一个 major 后删」);(c) 维持现状。
