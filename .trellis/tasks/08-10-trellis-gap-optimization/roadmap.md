...
3. **(新议题,已从本任务分出)归档记账覆盖率 23%** —— 复评任务轴时的账本数据顺带暴露: W3 后 `session_ended` 13 次而 `session_archive_attempted` 只有 3 次。可能是 Stop hook 没触发 / skill 早退没记账 / 归档真没发生,三种处置完全不同。**这是 W3「沉淀减负」的效果验收缺口,不属于 Trellis 对照这个任务的范围**,另开。
