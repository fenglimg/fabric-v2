...
- **CLI --help drift gate**（**新增**，TASK-003）— `packages/cli/__tests__/cli-surface.test.ts`，`fabric --help`、`fabric install --help`、`fabric scan --help`、`fabric doctor --help`、`fabric uninstall --help` 输出快照与 [docs/test-seed/cli.md](./test-seed/cli.md) §1 不一致时 CI 失败。这是种子 §1 与代码对账的唯一自动化通道。（v2.0.0-rc.37 起 `fabric serve` 已 quarantine，不在 drift gate 范围。）
