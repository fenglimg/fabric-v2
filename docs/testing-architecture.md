...
- **CLI --help drift gate**（**新增**，TASK-003）— `packages/cli/__tests__/cli-surface.test.ts`，`fab --help`、`fab install --help`、`fab scan --help`、`fab doctor --help`、`fab serve --help` 输出快照与 [docs/test-seed/cli.md](./test-seed/cli.md) §1 不一致时 CI 失败。这是种子 §1 与代码对账的唯一自动化通道。
