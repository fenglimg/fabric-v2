...
真正关键的是，Fabric 把“项目识别结果”和“下一步 AI handoff”放进了同一条链路。
...
Rules View 应该把 `AGENTS.md` 层级、revision hash、最近 `sync-meta`
