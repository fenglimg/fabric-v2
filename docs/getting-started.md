...
| 命令 | 作用 |
| --- | --- |
| `fab install` | TTY 下进入 wizard，确认计划后写入。 |
| `fab install --yes` | 非交互执行当前计划。 |
| `fab install --plan` | 只打印计划，不写文件。 |
| `fab install --reapply --yes` | 对已初始化仓库重应用 Fabric 管理文件。 |
...
`fab install` 默认非破坏性。已有 Fabric 产物时，除非显式使用 `--reapply` 或相关 force 行为，否则应中止或跳过冲突写入。
...
```text
我刚运行了 fab install，请继续完成这个仓库的 Fabric 初始化。
```
...
- `fab doctor --fix` 可从 `.fabric/rules/` 重建 `.fabric/agents.meta.json` 与 `.fabric/rule-test.index.json`。
...
- L2 规则可提供 `[BUSINESS_LOGIC_CHUNKS]` 记录反直觉业务约束；`fab doctor` 会诊断其中的 `Anchor` 是否 stale、duplicate 或 missing。
...
`.fabric/events.jsonl` 是唯一 ledger。`fab doctor --fix` 可以创建缺失的 events file，但不会迁移或投影其他 ledger。
...
`fab doctor` 默认只读。`--fix` 只修复可确定重建的派生状态：`.fabric/agents.meta.json`、`.fabric/rule-test.index.json`、缺失的 `.fabric/events.jsonl`、确定性的 bootstrap README 与 stale hashes。缺失 rule section、语义冲突、未完成的 init-context 确认、MCP client 本地配置问题和业务代码与规则不一致都属于人工处理范围。
