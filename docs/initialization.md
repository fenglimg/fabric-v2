...
5. 让 MCP、doctor 和 sync-meta 自动把 typed events 写入 `.fabric/events.jsonl`，不要在新流程中调用已废弃的 `fab_append_intent`。
...
Fabric requires MCP-capable client。更直接地说：如果 client 不能稳定调用 `fab_plan_context` 和 `fab_get_rule_sections`，并读取 `.fabric/events.jsonl` typed Event Ledger，它就无法完整执行 Fabric Protocol。`fab_update_registry` 和 `fab_append_intent` 是已废弃兼容 surface，新流程不应依赖它们。
