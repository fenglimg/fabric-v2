...
未声明 `activation` 的旧节点等价于 `path`，因此现有 `.fabric/agents.meta.json` 不需要迁移即可继续工作。Dashboard Rule Topology 会用同一套解析结果展示当前样本路径命中了哪些 L1/L2 规则，以及哪些节点只作为 description stub 出现。
...
## Rule Topology 与 Rules Context API
...
返回内容与 `fab_get_rules(path=...)` 的 rules payload 对齐，包含：
...
Dashboard 默认进入 Rule Topology 模块。维护者可以输入一个样本路径，查看目录覆盖热力图和命中原因面板；这适合验证 `scope_glob`、`activation.tier` 与 Shadow Mirroring 元数据是否按预期协同，而不是作为编辑规则的入口。
