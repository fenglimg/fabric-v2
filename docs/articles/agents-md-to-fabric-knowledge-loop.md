...
v2.0.0 起主线使用 stdio MCP，不需要启动独立 HTTP 服务。旧 `fabric serve` / REST / SSE / Dashboard 入口已 quarantine 到 `packages/server-http-experimental/`，仅作为历史参考。
...
- `fab_recall`：默认读路径，一次返回相关候选、`selection_token` 和完整知识正文；
- `fab_plan_context`：仅在响应过大或需要精确裁剪时使用，返回相关知识候选、`selection_token` 和描述索引；
- `fab_get_knowledge_sections`：两步流第二步，根据 `selection_token` 和选中的 `stable_id` 拉取具体知识正文；
- `fab_extract_knowledge`：把一次会话中值得保留的经验写入 `.fabric/knowledge/pending/<type>/`；
- `fab_archive_scan`：为 archive skill 提供会话/事件扫描候选；
...
左侧是 CLI，列出 fabric install、fabric store、fabric sync、fabric info、fabric doctor、fabric config、fabric uninstall。
上方是 AI Clients，包含 Claude Code、Codex CLI、Cursor。
右侧是 MCP，列出 fab_recall、fab_plan_context、fab_get_knowledge_sections、fab_extract_knowledge、fab_archive_scan、fab_review。
...
你在项目里修了一个认证模块的 bug。Agent 在修改 `src/auth/**` 前，PreToolUse hook 先给出路径相关的知识提示。Agent 再通过 `fab_recall` 查询和认证相关的历史 decision、pitfall、guideline；如果返回过大，才退回 `fab_plan_context` + `fab_get_knowledge_sections` 精确挑选正文。它发现之前有一条关于 token refresh 的坑，于是避免重复犯错。
...
2. 客户端连接：Claude Code / Codex CLI / Cursor 通过 stdio MCP 配置启动 server。
...
- 当前 CLI 公开命令包括 `install`、`store`、`sync`、`info`、`doctor`、`uninstall`、`config`；`plan-context-hint`、`onboard-coverage`、`metrics` 是隐藏/内部命令，`whoami` / `status` / `scope-explain` 是兼容别名。没有独立公开的 `fabric hooks install` 命令，hook 安装由 `fabric install` 编排完成。
- `fabric install` 的预览参数是 `--dry-run`，不是 `--plan`。
- 当前 MCP tool 名称是 `fab_recall`、`fab_plan_context`、`fab_get_knowledge_sections`、`fab_extract_knowledge`、`fab_archive_scan`、`fab_review`。
