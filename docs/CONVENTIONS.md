...
- `docs/` 只保留工程参考：上手、内部协议、源码地图、架构决策、开发约定、知识注册。
...
- 禁止把内部黑话放到主标题。
- 受保护 token 不翻译：`AGENTS.md`、`FABRIC.md`、`.fabric/knowledge/`、`.fabric/agents.meta.json`、`.fabric/events.jsonl`、`agent_meta`、`MUST`、`NEVER`。
...
- 每个 canonical knowledge entry 和核心文档节点必须有可稳定引用的 Stable ID。
- canonical knowledge entry 的身份来自 frontmatter `id` / `stable_id`，文件路径和 slug 不是身份真源。
- team entry 使用 `KT-*` 前缀；personal entry 使用 `KP-*` 前缀。
- `fabric doctor --fix` 负责 deterministic 派生状态；`fabric doctor --fix-knowledge` 负责 knowledge maintenance。
- Stable ID 变更必须同步 [RULE_REGISTRY](./RULE_REGISTRY.md)。

## Rule-Test 静态可追踪

- V1 只维护静态声明覆盖：测试文件用 `// @fabric-verify <stable_id>` 声明自己覆盖某条 knowledge-visible contract。
- `fabric doctor --fix` 负责扫描声明并生成派生 sidecar，同时保留 previous knowledge/test hash。
- `fabric doctor` 只做静态检查，报告 covered、stale_rule、stale_test、orphan、missing 等状态。
...
4. 不直接编辑 `.fabric/agents.meta.json`：知识正文变更必须落在 `.fabric/knowledge/` 或挂载 store，并通过 `fab_review` / `fabric-review` / `fabric doctor --fix-knowledge` 走治理路径。
