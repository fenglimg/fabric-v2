...
- 禁止把内部黑话放到主标题。
- 受保护 token 不翻译：`AGENTS.md`、`FABRIC.md`、`.fabric/agents/`、`.fabric/agents.meta.json`、`.fabric/human-lock.json`、`ledger_entry`、`agent_meta`、`shadow constraints`、`Shadow Mirroring`、`MUST`、`NEVER`。
...
- `sync-meta` 负责把 Stable ID 预编译进 `.fabric/agents.meta.json`。
...
- `fabric sync-meta` 负责扫描声明并生成 `.fabric/rule-test.index.json` sidecar，同时保留 previous rule/test hash。
...
4. 不直接编辑 `.fabric/agents.meta.json`：规则注册变更必须走 `fabric sync-meta` 或 `fabric doctor --fix` baseline 接受流程。
