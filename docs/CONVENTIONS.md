...
- `fabric doctor --fix` 负责把 Stable ID 从 `.fabric/rules/` 预编译进 `.fabric/agents.meta.json`。
...
- `fabric doctor --fix` 负责扫描声明并生成 `.fabric/rule-test.index.json` sidecar，同时保留 previous rule/test hash。
...
4. 不直接编辑 `.fabric/agents.meta.json`：规则正文变更必须落在 `.fabric/rules/`，再通过 `fabric doctor --fix` 接受派生 baseline。
