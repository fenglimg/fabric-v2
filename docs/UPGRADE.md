...
cd <your-project> && fab install             # 2. 每个项目重跑 (同步 SKILL.md / hooks / AGENTS.md)
```

### Verify
```bash
fab -v        # 2.0.0-rc.35
fab doctor    # hooks_wired ✓ 无 schema ERROR
```
...
| `fab doctor` 报 schema ERROR JSON dump | meta.json 旧字段命名 |

> `fab install` idempotent——同名 SKILL.md 会被新版本覆盖。手工改过的请提前备份。
