...
- release_signal: `npm publish @fenglimg/fabric-cli@1.0.0` 成功；干净项目上 `fabric init -> fabric serve` smoke test 通过且无需手工改文件。

### Scope

1. `fabric init` 向仓库写入最小 protocol skeleton，并一站式完成 bootstrap、MCP config 与 hooks 安装。
2. `fab scan` 及相关 CLI inspection 在 AI-assisted setup 之前生成 evidence。
3. MCP runtime dispatch 可通过 packaged server path 使用。
3. `fabric serve` 为维护者提供首个本地 control-plane session。
...
- release_signal: `fabric serve` 之后，Dashboard 在 `http://localhost:3333` 可达，成功加载项目状态，且单次本地 smoke test 可检查 ledger/rules 数据。
...
2. Feature #2: `fabric migrate`  
   在 schema 版本随时间变化时安全升级 `.fabric/` metadata。
3. Feature #3: `fabric doctor`  
