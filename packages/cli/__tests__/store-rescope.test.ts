...
describe("rescopeStore / promoteProjectToTeam (G-GUARD / A7)", () => {
  ...
  it("promote absorbs every project:* entry into team scope", () => {
    ...
    const report = promoteProjectToTeam(dir, { storeVisibility: "shared" });
  ...
  it("promote can target a single project, leaving other projects untouched", () => {
    const dir = makeStoreDir();
    registerProjects(dir, ["alpha", "beta"]);
    const a = seedEntry(dir, "KT-DEC-9001", "project:alpha");
    const b = seedEntry(dir, "KT-DEC-9002", "project:beta");

    const report = promoteProjectToTeam(dir, { projectId: "alpha", storeVisibility: "shared" });
    expect(report.changes).toHaveLength(1);
    expect(scopeOf(a)).toBe("team");
    expect(scopeOf(b)).toBe("project:beta");
  });

  it("refuses re-scoping to personal inside a shared store (R5#3) and leaves the file intact", () => {
    const dir = makeStoreDir();
    const file = seedEntry(dir, "KT-DEC-9001", "team");

    const report = rescopeStore(dir, "personal", { id: "KT-DEC-9001", storeVisibility: "shared" });
  ...
  it("refuses creating a dangling project ref (target project not registered)", () => {
    const dir = makeStoreDir();
    registerProjects(dir, ["alpha"]);
    const file = seedEntry(dir, "KT-DEC-9001", "team");

    const report = rescopeStore(dir, "project:ghost", { id: "KT-DEC-9001", storeVisibility: "shared" });
    expect(report.changes).toHaveLength(0);
    expect(report.refusals).toHaveLength(1);
  ...
  it("dry-run reports the change but writes nothing", () => {
    ...
    const report = rescopeStore(dir, "team", { id: "KT-DEC-9001", storeVisibility: "shared", dryRun: true });
    expect(report.dryRun).toBe(true);
    expect(report.changes).toHaveLength(1);
    expect(scopeOf(file)).toBe("project:alpha"); // untouched on disk
  });

  it("counts entries already at the target scope as unchanged", () => {
    const dir = makeStoreDir();
    seedEntry(dir, "KT-DEC-9001", "team");

    const report = rescopeStore(dir, "team", { storeVisibility: "shared" });
    expect(report.changes).toHaveLength(0);
