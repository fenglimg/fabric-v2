...
- `store remove` 是 *detach ≠ delete*：从 registry 卸载但保留磁盘 git 树。
- `store add` 拒绝磁盘无 store 树的 uuid (无「幽灵挂载」) —— 先 clone (`fabric install --global --url <remote>`) 或 `store create`。
- Personal-scope 写永远落在隐式 personal store，与 active write store 无关。
- Hook/skill 绝不直接解析 store 或执行 store 内文件 (S65 RCE 防线：store 是数据-only)；所有 store 状态经 CLI JSON 输出获取。
