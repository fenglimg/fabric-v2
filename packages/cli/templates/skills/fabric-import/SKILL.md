---
name: fabric-import
description: 冷启动从 git log + docs/*.md 回灌 active write store 的 knowledge/pending (NOT code/data import). Triggers 导入历史/bootstrap fabric/mine changelog/挖掘 commit.
...
One-time per-project cold-start: lift git commits + `docs/*.md` into `knowledge/pending/` as `team`-layer entries. Bridges `fabric install`'s 4-7 baseline → accumulated corpus. Run once on adoption or after major refactor.
