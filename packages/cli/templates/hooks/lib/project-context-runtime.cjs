...
// ../shared/src/schemas/store.ts
var STORE_UUID_PATTERN = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/u;
var storeUuidSchema = external_exports.string().regex(STORE_UUID_PATTERN, "store_uuid must be a canonical lowercase UUID");
var STORE_ALIAS_PATTERN = /^(?!\.{1,2}$)[A-Za-z0-9._-]{1,80}$/u;
var storeAliasSchema = external_exports.string().regex(
  STORE_ALIAS_PATTERN,
...
external_exports.object({
  ...
  allowed_scopes: external_exports.array(external_exports.string()).optional()
}).strict();
var STORE_PROJECT_ID_PATTERN = /^[a-z0-9_-]+$/u;
var STORE_MOUNT_NAME_PATTERN = /^[a-z0-9][a-z0-9._-]{0,78}[a-z0-9]$/u;
var storeMountNameSchema = external_exports.string().regex(
...
var storeProjectSchema = external_exports.object({
  // Single scope segment forming the `project:<id>` coordinate. Immutable.
  id: external_exports.string().regex(STORE_PROJECT_ID_PATTERN, "project id must be a single lowercase [a-z0-9_-] segment"),
...
external_exports.object({
  projects: external_exports.array(storeProjectSchema).default([])
}).strict();
var requiredStoreEntrySchema = external_exports.object({
  id: external_exports.string().min(1),
  suggested_remote: external_exports.union([external_exports.string().min(1), external_exports.literal(PERSONAL_STORE_SENTINEL)]).optional()
}).strict();
var mountedStoreSchema = external_exports.object({
  ...
  personal: external_exports.boolean().optional(),
...
external_exports.object({
  ...
  stores: external_exports.array(mountedStoreSchema).optional().default([]),
...
external_exports.object({
  ...
  credibility_half_life_models_days: external_exports.number().int().min(1).max(3650).optional(),
  credibility_half_life_pitfalls_days: external_exports.number().int().min(1).max(3650).optional(),
  credibility_half_life_processes_days: external_exports.number().int().min(1).max(3650).optional(),
  // Credibility floors (per maturity).
  credibility_floor_draft: external_exports.number().min(0).max(1).optional(),
  credibility_floor_verified: external_exports.number().min(0).max(1).optional(),
  credibility_floor_proven: external_exports.number().min(0).max(1).optional(),
  // Orphan-demote inactivity thresholds (per maturity).
  orphan_demote_proven_days: external_exports.number().int().min(1).max(3650).optional(),
  orphan_demote_verified_days: external_exports.number().int().min(1).max(3650).optional(),
  orphan_demote_draft_days: external_exports.number().int().min(1).max(3650).optional()
}).passthrough();

// ../shared/src/schemas/scope.ts
var SCOPE_COORDINATE_PATTERN = /^[a-z0-9_-]+(:[a-z0-9_-]+)*$/u;
var scopeCoordinateSchema = external_exports.string().min(1).regex(
...
external_exports.object({
  ...
  visibility_store: external_exports.string().min(1)
}).strict();

// ../shared/src/schemas/fabric-config.ts
...
var auditModeSchema = external_exports.enum(["strict", "warn", "off"]);
var clientPathsSchema = external_exports.object({
  claudeCodeCLI: external_exports.string().optional(),
  claudeCodeDesktop: external_exports.string().optional(),
  codexCLI: external_exports.string().optional()
}).strict();
var mcpPayloadLimitsSchema = external_exports.object({
  warnBytes: external_exports.number().int().positive().optional(),
  hardBytes: external_exports.number().int().positive().optional()
}).optional();
var selectionTokenTtlMsSchema = external_exports.number().int().min(3e4).max(36e5);
var planContextTopKSchema = external_exports.number().int().min(1).max(200);
...
var observeConfigSchema = external_exports.object({
  ...
  stop: external_exports.boolean().optional()
}).strict();
var writeRouteSchema = external_exports.object({
...
var fabricConfigSchema = external_exports.object({
  ...
  workspace_binding_id: external_exports.string().optional(),
  ...
  required_stores: external_exports.array(requiredStoreEntrySchema).superRefine(refineMaxOneTeamStore).optional(),
  ...
  active_write_store: external_exports.string().optional(),
  ...
  active_project: external_exports.string().optional(),
  ...
  write_routes: external_exports.array(writeRouteSchema).optional(),
  default_write_store: external_exports.string().optional(),
  ...
  // documented above.
  scanIgnores: external_exports.array(external_exports.string()).optional(),
  ...
  archive_hint_cooldown_hours: external_exports.number().int().positive().optional().default(12),
  ...
  underseed_node_threshold: external_exports.number().int().positive().optional().default(10),
  ...
  archive_edit_threshold: external_exports.number().int().positive().optional().default(20),
  ...
  archive_hint_hours: external_exports.number().int().positive().optional().default(24),
  ...
  review_hint_pending_count: external_exports.number().int().positive().optional().default(10),
  ...
  review_hint_pending_age_days: external_exports.number().int().positive().optional().default(7),
  ...
  maintenance_hint_days: external_exports.number().int().positive().optional().default(14),
  ...
  maintenance_hint_cooldown_days: external_exports.number().int().positive().optional().default(7),
  ...
  review_stale_pending_days: external_exports.number().int().positive().optional().default(14),
  ...
  cite_recall_nudge: external_exports.boolean().optional().default(true),
  cite_recall_window_minutes: external_exports.number().int().min(0).optional().default(30),
  ...
  // `**` = across segments.
  cite_nudge_ignore_globs: external_exports.array(external_exports.string()).optional(),
  ...
  conflict_lint_similarity_threshold: external_exports.number().min(0).max(1).optional().default(0.5),
  ...
  // misconfig.
  fabric_event_retention_days: external_exports.union([external_exports.literal(7), external_exports.literal(30), external_exports.literal(90)]).optional(),
  ...
  onboard_slots_opted_out: external_exports.array(external_exports.string()).optional().default([]),
  ...
  broad_index_backstop: external_exports.number().int().min(20).max(500).optional().default(50),
  ...
  cite_policy_enabled: external_exports.boolean().optional().default(true),
  self_archive_policy_enabled: external_exports.boolean().optional().default(true),
  ...
  embed_enabled: external_exports.boolean().optional().default(true),
  ...
  embed_weight: external_exports.number().int().min(0).max(49).optional().default(30),
  ...
  fusion: external_exports.enum(["additive", "rrf", "auto"]).optional().default("auto")
});
var fabricConfigLoadSchema = fabricConfigSchema.merge(
  external_exports.object({
    required_stores: external_exports.array(requiredStoreEntrySchema).optional()
  })
);

// ../shared/src/store/project-config-io.ts
function projectConfigPath(projectRoot) {
  return path.join(projectRoot, ".fabric", "fabric-config.json");
}
...
var ProjectContextAmbiguousError = class extends Error {
  ...
  workspaceRoots;
  code = "FABRIC_PROJECT_CONTEXT_AMBIGUOUS";
};
var PROJECT_ROOT_SIGNALS = ["env", "marker", "cwd", "repo"];
var projectRootSignalSchema = external_exports.enum(PROJECT_ROOT_SIGNALS);
var projectRootSignalsSchema = external_exports.object({
  ...
  markerDir: external_exports.string().optional(),
  ...
  discoveredProjectId: external_exports.string().optional()
}).strict();
var projectRootResolutionSchema = external_exports.object({
...
var storeResolverWarningCodeSchema = external_exports.enum(STORE_RESOLVER_WARNING_CODES);
var storeResolverWarningSchema = external_exports.object({
  code: storeResolverWarningCodeSchema,
  // The alias/UUID/id the warning concerns.
  ref: external_exports.string().min(1),
  message: external_exports.string().min(1)
}).strict();
var readSetEntrySchema = external_exports.object({
  store_uuid: external_exports.string().min(1),
...
var storeReadSetSchema = external_exports.object({
  stores: external_exports.array(readSetEntrySchema),
  warnings: external_exports.array(storeResolverWarningSchema)
}).strict();
var writeTargetSchema = external_exports.object({
  store_uuid: external_exports.string().min(1),
  alias: external_exports.string().min(1)
}).strict();
var storeResolveInputSchema = external_exports.object({
  // Machine identity (S33) — namespaces personal ids; identifies personal store.
  uid: external_exports.string().min(1),
  // Stores mounted on this machine (from global config).
  mountedStores: external_exports.array(
    external_exports.object({
      store_uuid: external_exports.string().min(1),
      alias: external_exports.string().min(1),
      mount_name: external_exports.string().min(1).optional(),
      remote: external_exports.string().min(1).optional(),
      writable: external_exports.boolean().default(true),
      // Marks the implicit personal store.
      personal: external_exports.boolean().default(false)
    }).strict()
  ),
  // The project's declared required_stores (ids/aliases + optional remote).
  requiredStores: external_exports.array(
    external_exports.object({
      id: external_exports.string().min(1),
      suggested_remote: external_exports.string().min(1).optional()
    }).strict()
  ),
  // Alias selected as the active write store for non-personal scopes, if any.
  activeWriteAlias: external_exports.string().min(1).optional(),
  ...
  activePersonalAlias: external_exports.string().min(1).optional(),
  // Scope-aware write routes. Exact scope wins first, then longest prefix route.
  writeRoutes: external_exports.array(
    external_exports.object({
      scope: external_exports.string().min(1),
      store: external_exports.string().min(1)
    }).strict()
  ).optional().default([]),
  defaultWriteAlias: external_exports.string().min(1).optional()
}).strict();
var projectRootGoldenCaseSchema = external_exports.object({
...
external_exports.object({
  contract: external_exports.literal("project-root.golden"),
  cases: external_exports.array(projectRootGoldenCaseSchema).min(1)
}).strict();
var readSetGoldenCaseSchema = external_exports.object({
  name: external_exports.string().min(1),
  note: external_exports.string().optional(),
  input: storeResolveInputSchema,
  // Scope under test for the write-target expectation.
  writeScope: external_exports.string().min(1),
  expected: external_exports.object({
    readSet: storeReadSetSchema,
    writeTarget: writeTargetSchema.nullable(),
    writeWarnings: external_exports.array(storeResolverWarningSchema)
  }).strict()
}).strict();
external_exports.object({
  contract: external_exports.literal("read-set.golden"),
  cases: external_exports.array(readSetGoldenCaseSchema).min(1)
}).strict();

// ../shared/src/schemas/bindings-snapshot.ts
external_exports.object({
  ...
  workspace_binding_id: external_exports.string().min(1),
  ...
  knowledge_stats: external_exports.object({
    pending_count: external_exports.number().int().nonnegative(),
    canonical_count: external_exports.number().int().nonnegative(),
    oldest_pending_mtime_ms: external_exports.number().nonnegative().nullable()
  }).strict().optional(),
...
// ../shared/src/resolver/project-context-resolver.ts
function canonicalCandidate(raw, cwd) {
...
function findProjectMarker(start) {
  ...
  for (let depth = 0; depth < 64; depth += 1) {
    if (fs.existsSync(path.join(current, ".fabric", "fabric-config.json"))) return current;
...
function createProjectContextResolver(input = {}) {
  const cwd = input.cwd ?? process.cwd();
  let source;
  let rawCandidates;
  if (input.explicitRoot !== void 0) {
    source = "explicit-pin";
    rawCandidates = [input.explicitRoot];
  } else if (input.roots !== void 0) {
    source = "client-root";
