...
export type StoreDiagnosticCode =
  ...
  // config-single-home W4: a repo's fabric-config.json still carries a relocated
  // policy key. The repo file is identity-only now, so the value has no effect —
  // INFO advisory pointing at its new home (global defaults / store-config).
  | "config_key_relocated"
