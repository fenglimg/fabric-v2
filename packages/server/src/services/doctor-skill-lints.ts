...
export type RouterChainRefInspection = {
  status: "ok" | "drift";
  // Leaf slugs referenced by the router's S_CHAIN that are NOT in the valid
  // leaf set (the drift the lint catches). Empty when status === "ok".
  unknownRefs: string[];
};
...
const FABRIC_SKILL_SLUGS = ["fabric-archive", "fabric-review", "fabric-import"] as const;

// B2 skill-router (A4): the valid downstream leaf skills the fabric/ router may
// chain to in its S_CHAIN section. Mirrors the 7 leaf slugs installed by
// FABRIC_SKILL_INSTALL_SPECS (the CLI's install source of truth). Kept as a
// local constant rather than imported from the CLI to preserve the
// server-does-not-depend-on-cli direction. If a leaf is added/renamed in
// install, this set must be updated in lockstep — the lint exists to catch the
// inverse drift (an S_CHAIN reference to a leaf that no longer exists).
const ROUTER_VALID_LEAF_SLUGS: ReadonlySet<string> = new Set([
  "fabric-archive",
  "fabric-review",
  "fabric-import",
  "fabric-sync",
...
/**
 * B2 skill-router (A4): backstop lint for the fabric/ router's `S_CHAIN`
 * section — the hand-authored auto-chain table that is NOT regenerated by
 * install (only the Intent Map marker block is). Every backtick-quoted
 * `fabric-*` token in S_CHAIN must reference a real leaf skill in
 * {@link ROUTER_VALID_LEAF_SLUGS}; a reference to a removed/renamed leaf is
 * drift the generated Intent Map cannot catch.
 *
 * Reads whichever client copy is present (both are byte-identical). Missing
 * router install or missing S_CHAIN section → ok no-op (mirrors the sibling
 * skill lints' missing-file tolerance). Warning kind — `fabric install` does
 * not auto-fix hand-authored prose, so remediation is manual.
 */
export async function inspectRouterChainRef(projectRoot: string): Promise<RouterChainRefInspection> {
  const candidatePaths = [
    join(projectRoot, ".claude", "skills", "fabric", "SKILL.md"),
    join(projectRoot, ".codex", "skills", "fabric", "SKILL.md"),
  ];
...
export function createRouterChainRefCheck(t: Translator, inspection: RouterChainRefInspection): DoctorCheck {
