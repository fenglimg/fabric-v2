# Fabric v2.1 全局多 store 重构 — 实现 Roadmap v3

> Source: `.workflow/.maestro/20260529-global-refactor-impact/status.json`（审计 66 surface / 7 维度 design-complete）
> v3 changelog（应用 round-2 cross-LLM 冷评 gemini 零上下文 ~89 / codex 接地验证锚 ~89）：
> ①**杀自举悖论**：拆 P0(纯契约/schema,不碰真实 HOME) → P0.5(测试墙) → P0.6(resolver TDD 实现),依赖方向理顺(gemini#1)
> ②**P5/P6 验收去虚指**：parity-matrix 契约文件 + perf 数值阈值 + redaction 样例(gemini#3 ≡ codex#3 共识)
> ③**覆盖表纯化**：A/F/E/R5# 决策/反馈锚拆出独立小节,surface 表只留 S*(codex#1)
> ④重复 surface 标 primary/secondary(codex#2) ⑤P3 内声明实现顺序(codex#4) ⑥顶部加显式 DAG(codex#5)
> ⑦P2 写工具依赖澄清(gemini#2 经 codex 接地拓扑核验 largely refuted,仅补依赖注释不重排)
> 排序原则：**契约 → 测试墙 → resolver 实现 first**；clean-slate（S22）；每 phase 回指 surface + 可验 done_when。

## 依赖 DAG（执行器按锚点调度）

```
P0(契约/schema) → P0.5(测试墙) → P0.6(resolver TDD 实现)
                                      ↓
                                  P1(多 store + git 核心)
                                      ↓
                          ┌───────────┴───────────┐
...
无环；P0→P0.5→P0.6 是唯一关键路径头；P2 与 P3 可部分并行(P2 写工具依赖 P1 默认 store 物理写,不依赖 P3 多 store CLI)。

---

## Phase 0 — Schema + Resolver 契约（纯定义层，不碰真实 HOME，阻塞全部）

**目标**：把"项目根解析 + 多 store/scope/身份/读写"落成 **schema 类型 + 两 resolver 接口契约 + golden fixture 规格**。本 phase 只产类型/签名/规格,不跑真实 HOME/git —— 实现在 P0.6 TDD。

- **ProjectRootResolver 契约（多信号，S15/S32/S45）**：定义解析序 `env FABRIC_PROJECT_ROOT > 向上探 .fabric/fabric-config.json marker > cwd > repo marker`；一仓一 .fabric 一 project_id（S32）；worktree 靠 project_id 归并（S45）—— 本 phase 出**接口 + 四信号 golden case 规格**
...
**done_when**：4 schema 过 zod 编译；两 resolver TS 接口 + 四信号(env/marker/cwd/repo) golden case **规格文件**写出(JSON fixture 期望值,尚不跑);同构布局类型可被 reader 类型识别。**不要求**真实 HOME 跑通(那是 P0.6)。
...
- 把 P0 写出的 read-set / write-target / projectRoot golden **规格**实例化为可跑断言(尚无实现 → 预期全红,作 TDD 起点)

**done_when**：隔离 HOME + fake remote + 三端 fixture 就位,CI 在隔离 HOME 下能跑(P0 golden 断言此时为预期红);旧布局 negative test 确认不读。
**依赖**：Phase 0（仅 schema/契约类型,非实现）。

## Phase 0.6 — Resolver 实现（TDD，把 P0.5 红灯转绿）

**目标**：在 P0.5 测试墙上实现 ProjectRootResolver + StoreResolver,直到 P0 golden case 全绿。

- 实现四信号 projectRoot 解析(S15/S32/S45)
- 实现 read-set / write-target 计算(S11/S54/S60)、store alias+UUID 解析(S55)、缺失 store warnings(S51)

**done_when**：P0.5 中 read-set/write-target/projectRoot golden case 全绿;隔离 HOME 下 CI 绿。
**依赖**：Phase 0/0.5。
...
> **依赖澄清（addr gemini#2,codex 拓扑核验）**：本 phase 写工具(archive/extract)写入 **P1 默认 store 物理写 + P0/P0.6 write-target resolver**,**不依赖** P3 的 `store add/bind` 多 store CLI。多 store 写目标在 P3 后自然扩展,无需把 P2 后置。
...
- MCP server cwd→project 行为（接 Phase 0.6 ProjectRootResolver）→ S15-mcp
...
> **实现顺序（addr codex#4）**：① install transaction(plan/apply/verify/rollback+receipt) → ② store lifecycle(list/add/remove/bind/switch-write/explain) → ③ sync 冲突/离线状态机 → ④ status/scope-explain/whoami/doctor。后件依赖前件就绪。
...
**done_when（去虚指,addr codex#3/gemini#3）**：
- secret-scan/lint 已在 P2 上线（此处只接审计追踪）
- **三端 parity 由契约文件 `parity-matrix.json` 驱动**（每行=一能力×三端期望态）,验收 = 矩阵 100% E2E 用例绿,非人工"对等"判断
...
> 重复落点已标 `(primary)` / `(secondary)`（addr codex#2）。非 surface 决策/反馈锚见下方独立小节(addr codex#1)。

- **P0（契约/schema）**: S11 · S13(config,primary) · S15(resolver,primary) · S20 · S23 · S27 · S32 · S33 · S42(schema,primary) · S45 · S54 · S55 · S59 · S60
- **P0.5**: S39 · S22(negative,primary) · S66(legacy-negative,secondary)
- **P0.6**: (实现 P0 的 S11/S15/S54/S60 等,不新增 surface)
- **P1**: S2 · S12 · S16 · S18 · S42(物理,secondary) · S43 · S58 · S66(legacy 边界,primary)
- **P2**: S15(MCP cwd,secondary) · S21 · S26(gate,primary) · S49(lint,primary) · S53 · S61 · S15-mcp
- **P3**: S1 · S4 · S5 · S8 · S9 · S10 · S13(project_id,secondary) · S17 · S19 · S24 · S25 · S28 · S30 · S34 · S36 · S37 · S47 · S51 · S57 · S7
- **P4**: S3 · S14(skill,primary) · S29(skill,primary) · S44 · S46 · S50 · S62 · S63 · S64 · S65 · S66(改造,secondary)
- **P5**: S6 · S26(trail,secondary) · S38 · S41 · S49(已 P2,secondary) · S14(parity,secondary) · S29(parity,secondary)
- **P6**: S35 · S40
- **Deferred**: S31 · S48 · S52 · S56
- 核对：S1-S66 全部落点,S11 已补,无裸"跨 phase",重复面已标主次。

## 决策/反馈锚 → phase（非 surface，独立追溯，addr codex#1）
