...
4. **Duplicate of existing canonical** — v2.0.0-rc.37 NEW-4: this check is now **mandatory**. Pre-PASS MUST step: for each candidate, query mounted store-backed knowledge keyed on slug-stem / stable_id / summary; do NOT `Glob('.fabric/knowledge/**/*.md')` in the project root. If duplicate found → drop candidate. Silently writing a near-duplicate is the highest-noise failure mode.
